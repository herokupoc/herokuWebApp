CREATE OR REPLACE FUNCTION herokupay.hc_contact_status()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
                    BEGIN
                      ...
                      ELSE  -- connect op
                        ...
                        NEW._hc_err = NULL;
                        	INSERT INTO util.t_util_log (insert_time, log_locale, log_msg) VALUES(current_timestamp, 'herokupay TG_OP:', TG_OP);
		                    ...
		                    if(NEW._hc_lastop = 'SYNCED') then
		                    	...
		                    	--INSERT/OR/UPDATE CENTRAL
		                  		if (coalesce(OLD._hc_lastop,'SYNCED') = 'SYNCED' and (TG_OP = 'INSERT' or TG_OP = 'UPDATE'))  THEN 
									...
									perform central_sf_consolidate(TG_OP ,'SF-TRANSVERSAL' ,new.sfid , new.ownerid, new.accountid, new.salutation ,
                          		...
                          		if(OLD._hc_lastop = 'INSERTED' and TG_OP = 'UPDATE') then 
			                        ...
			                        perform central_sf_consolidate('INSERT','SF-TRANSVERSAL' ,new.sfid , new.ownerid, new.accountid, new.salutation ,
		    	              			new.firstname, new.middlename, new.lastname, new.mobilephone, new.acu_workphone__c, current_timestamp, 
		        	          			new.title, new.email, new.acu_decisionmaker__c, new.isdeleted, new.lastmodifiedbyid, NEW.createddate); 	
	                	    end if;
